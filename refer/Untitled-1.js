...
// const options = [
// ["logOptions","1About this mac","System Preferences","1App Store","Sleep","Restart","1Shut Down","Log Out"]
// ["Finder","1About Finder","1Preferences","31Empty Trash",["1Services","3Images","1Import Images","3Development","Activity Monitor","Allocations & Leaks","File Activity","System Trace","Activity Monitor","Time Profile Active Application","Time Profile App Under Mouse","Time Profile Entire System","Toggle Instruments Recording"],"Hide Others","Show All",""],
// ];
...
$(function () {
	...
	$(".canCheck").click(function () {
		...
		var sAnimateAppsIconOnOpen = $(".sAnimateAppsIconOnOpen").is(
			":checked"
		);
		if (sAnimateAppsIconOnOpen) {
			$(".taskbar").addClass("animateOnOpen");
	...
	$(".canCheck .status").click(function () {
		...
		if ($(this).hasClass("sEnergySaver")) {
			if (
				$(this).hasClass("sEnergySaver") & $(this).hasClass("checked")
			) {
	...
	$(document).bind("contextmenu", function (event) {
		...
		let cmSep = '<div class="sep"></div>',
			...
			cmFilesysGetInfo =
				'<div class="item sFilesysGetInfo">Get Info</div>',
			cmFilesysInfo =
			...
			cmNotDefined =
				'<div class="item">Work In Progress (Be patient)</div>';
   ...
   $('.taskbar__item[data-window="' + appName + '"]').addClass('taskbar__item--active');
	...
	function centerApp(e) {
		...
		var targetWindow = $(
			'.window[data-window="' + $(this).data("window") + '"]'
		);
		var setTop =
	...
	$(".window__controls").each(function () {
		...
		$(this)
			...
			.click(function (e) {
				...
				setTimeout(function () {
					$('.taskbarApp[data-window="' + appName + '"]').removeClass(
						"open"
					);
					$(
						'.taskbar__item[data-window="' + appName + '"]'
					).removeClass("taskbar__item--open taskbar__item--active");
				}, 1);
			});
	...
	if (target.find(".close").is(":hover")) {
		if (target.hasClass("active")) {
			...
			if (
				...
				$(".content[data-tabId='" + nextTabId + "']").addClass(
					"visible"
				);
			}
		}
	...
	documents: [
		...
		},
		{
			...
			c: [
				{
					t: "xlsx",
					out: "Hi I´m Varo, your local crazy developer, and I am trying to make an online functional OS (yeah, I am sooooo bored)",
				},
			],
		},
	],
...
function AppFilesUpdateCurrentItem() {
	...
	AppFilesCurrentItem = $(".window--files .sidebar .item.active").data(
		"item"
	);
